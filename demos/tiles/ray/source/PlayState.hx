...
class PlayState extends FlxState
{
	...
	override public function create()
	{
		...
		tween = FlxTween.circularMotion(target, center.x, center.y + halfHeight, FlxG.height / 2 - halfHeight, 0, false, 4, true, {type: FlxTweenType.LOOPING});
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (reachedTarget)
		{
			FlxSpriteUtil.drawLine(line, center.x, center.y, target.x, target.y, {thickness: 3, color: 0xFF00FF00});
		}
		else
		{
			FlxSpriteUtil.drawLine(line, center.x, center.y, hitPoint.x, hitPoint.y, {thickness: 3, color: 0xFFFF0000});
		}
	}
